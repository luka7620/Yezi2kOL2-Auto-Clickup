...
def organize_output():
    ...
    with open(usage_file, 'w', encoding='utf-8') as f:
        ...
        f.write("第2步：按四个分页完成配置：\n")
        f.write("  - 基础设置：按键精灵路径、窗口关键词、两个按钮文本\n")
        f.write("  - 时间计划：开始/结束时间和生效星期\n")
        f.write("  - 高级参数：开机延迟、启动等待、重试次数、重试间隔、按钮等待\n")
        f.write("  - 运行选项：是否显示进度窗口、是否保持目标窗口置顶\n\n")
        ...
        f.write("2. config.json 文件固定创建在程序所在目录，不受启动工作目录影响\n")
